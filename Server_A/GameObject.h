...
public:
    Common::GameObjectType GetGameObjectType() const { return _objectType; }

    RoomRef GetRoom() { return _room; }
    void SetRoom(RoomRef room) { _room = room; }

    Common::OBJECT_INFO& GetObjectInfo()  { return _info; }
    //현재는 참조로 받아서 관리하는데 문제 생길시 값 또는 수정
    void SetObjectInfo(Common::OBJECT_INFO& info) { _info = info; }

    Common::MoveDir GetMoveDir() const { return _info.posinfo().movedir(); }
    void SetMoveDir(Common::MoveDir movedir) { _info.mutable_posinfo()->set_movedir(movedir); }
    ...
    Common::CreatureState GetState()const { return _info.posinfo().state(); }
    void SetState(Common::CreatureState state) { _info.mutable_posinfo()->set_state(state); }

    int32 GetObjectId() const { return _info.objectid(); }
    void SetObjectId(int32 id) { _info.set_objectid(id); }

    Common::STATINFO GetObjectStat() const { return _info.statinfo(); }
    void SetObjectStat(Common::STATINFO stat) { _info.mutable_statinfo()->CopyFrom(stat); }
    ...
    Common::POSITIONINFO GetPosinfo() const { return _info.posinfo(); }

    Vector2Int GetFrontCellPos(Common::MoveDir dir);
    Vector2Int GetFrontCellPos();

    static Common::MoveDir GetDirFromVec(Vector2Int dir);
...
protected:
    ...
    void SetGameObjectType(Common::GameObjectType objectType) { _objectType = objectType; }

private:

private:
    Common::GameObjectType _objectType = Common::GameObjectType::NONE;
    RoomRef _room;
    Common::OBJECT_INFO _info;
};
